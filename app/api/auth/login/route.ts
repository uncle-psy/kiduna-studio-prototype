...
export async function POST(request: Request) {
  ...
  const [user] = await sqlClient<{ id: string; password_hash: string; email_verified_at: Date | null; status: string }[]>`
    select id, password_hash, email_verified_at, status from prototype_users where email = ${email} limit 1
  `;
