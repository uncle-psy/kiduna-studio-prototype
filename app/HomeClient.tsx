...
type Props = { account: { name: string; email: string } | null; verification: string | null };
...
export default function HomeClient({ account, verification }: Props) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = await response.json() as { error?: string; message?: string; prototypeVerificationUrl?: string };
      if (!response.ok) throw new Error(payload.error || "That didn’t work.");
      if (mode === "login") window.location.assign("/studio");
  ...
  return <main className={styles.home}>
    ...
    <section className={styles.access}>
      {account ? <div className={styles.returning}>
        ...
        <a href="/studio">Enter the Field <span>→</span></a><button onClick={logout}>Sign out</button>
      </div> : <>
        ...
        <form onSubmit={submit}>
          ...
          <label><span>Password</span><input name="password" type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} minLength={mode === "signup" ? 10 : undefined} required /></label>
          ...
          <button className={styles.submit} type="submit" disabled={busy}>{busy ? "Please wait…" : mode === "login" ? "Enter the Field" : "Create account & verify email"}</button>
        </form>
