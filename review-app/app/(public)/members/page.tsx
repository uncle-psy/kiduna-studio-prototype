...
/* ── Count DUNAs joined from the "Joined" link value ── */
function countJoined(p: PersonDef): number {
  const joined = p.links.find((l) => l.label === 'Joined')
  if (!joined) return 0
  return joined.value.split(',').length
}
...
const SORT_BUTTONS: { field: SortField; label: string }[] = [
  ...
  { field: 'count', label: 'DUNAs joined' },
  { field: 'role', label: 'Role' },
]

export default function MembersPage() {
  const c = ALLIES_PAGES.members
  ...
  useEffect(() => {
    async function load() {
      try {
        const rows = await listDirectory({ role: 'member', limit: 200 })
    ...
    if (sortField) {
      list.sort((a, b) => {
        ...
        switch (sortField) {
          ...
          case 'count':
            va = countJoined(a); vb = countJoined(b); break
          case 'role':
      ...
      <SectionWrapper style={{ paddingTop: 28 }}>
        ...
        {loading && (
          <div className="text-center py-16">
            ...
            <p className="text-muted text-[0.95rem]">Loading members…</p>
          </div>
        )}
        ...
        {!loading && !error && (
          <>
            ...
            {filtered.length > 0 ? (
              ...
              <div className="text-center py-16 rounded-[14px] border border-border" style={{ background: 'var(--surface, #0A0D33)' }}>
                <p className="text-muted text-[1rem]">No published members yet. Be the first to join!</p>
              </div>
            ) : (
              <div
              ...
              >
                <p className="text-muted text-[1rem]">
                  No members match &ldquo;{query}&rdquo;. Try another search.
                </p>
              </div>
            )}
