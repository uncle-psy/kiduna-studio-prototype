...
export const metadata: Metadata = {
  title: 'Service Alliance ($SRVA) — WV DUNA',
  description:
    'Service Alliance on WV DUNA — Veterans · Forming. Verified digital infrastructure helping veterans navigate benefits, health, and the transition to civilian work, with agents guiding each case.',
}
...
export default function SRVAPage() {
  return (
    <>
      <LandingHeader />
      ...
      <section style={{ paddingTop: 30, paddingBottom: 80 }}>
        <div className="w-full max-w-[1180px] mx-auto px-6">
          ...
          <div className="srva-detail-grid">
            <style>{`
              .srva-detail-grid { display: grid; grid-template-columns: 1.7fr 1fr; gap: 28px; align-items: start; }
              @media (max-width: 860px) { .srva-detail-grid { grid-template-columns: 1fr; } .srva-panel { position: static !important; } }
              .srva-stats { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0; background: linear-gradient(135deg, #100E59, #0A0D33); border: 1px solid var(--border, rgba(255,255,255,0.12)); border-radius: 14px; overflow: hidden; }
              @media (max-width: 900px) { .srva-stats { grid-template-columns: repeat(2, 1fr); } }
              @media (max-width: 480px) { .srva-stats { grid-template-columns: 1fr; } .srva-stats > div { border-right: 0; border-bottom: 1px solid var(--border, rgba(255,255,255,0.12)); } }
              .srva-cards { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; }
              @media (max-width: 640px) { .srva-cards { grid-template-columns: 1fr; } }
            `}</style>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 4 }}>
                ...
                <div>
                  ...
                  <p style={{ color: 'var(--fg-muted, #CDCDCD)', fontSize: '0.95rem', margin: '0.25rem 0 0' }}>
                    2nd Cavalry Association · Veterans · Private
                  </p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
                    <Chip>Forming</Chip>
                    <Chip>Veterans</Chip>
                    <Chip>WV SoS · Jan 2026</Chip>
                    <Chip>$SRVA</Chip>
                  </div>
                </div>
              </div>
              ...
              <p style={{
                ...
                Verified digital infrastructure helping veterans navigate benefits, health, and the transition to civilian work, with agents guiding each case.
              </p>

              {/* Stats bar */}
              <div className="srva-stats" style={{ marginTop: 8 }}>
                <Stat num="9,640" label="Members" />
                <Stat num="$880k" label="Treasury" />
                <Stat num="Private" label="Access" />
                <Stat num="$SRVA" label="Token" numStyle={{ fontSize: '1.5rem' }} />
              </div>

              {/* Two info cards */}
              <div className="srva-cards" style={{ marginTop: 22 }}>
                <InfoCard kicker="How it works" title="Member-owned, agent-run">
                  Service Alliance is governed by its members and operated alongside intelligent agents that handle the routine work. Every action settles on-chain and is recorded on a public ledger.
                </InfoCard>
                <InfoCard kicker="Legal standing" title="Registered in West Virginia">
                  Registered with the West Virginia Secretary of State, Service Alliance has a public legal home, a liability shield for members, and standing to contract, hold a treasury, and operate worldwide.
                </InfoCard>
              </div>
              ...
              <div style={{
                ...
                <span style={{ fontFamily: 'var(--font-display)', color: 'var(--accent, #EAAA00)', fontSize: '1.3rem' }}>
                  Founded via 2nd Cavalry Association
                </span>
                <span style={{ color: 'var(--fg-muted, #CDCDCD)', fontSize: '0.95rem' }}>
                  Registered Jan 2026 · West Virginia Secretary of State
                </span>
            ...
            >
              ...
              <p style={{ color: 'var(--fg-muted, #CDCDCD)', fontSize: '0.92rem', margin: '0.4rem 0 1rem' }}>
                Members-only for now. The token hasn&rsquo;t launched yet, so participation is by membership and mutual consent.
              </p>

              <PanelRow label="Treasury" value="$880k" />
              <PanelRow label="Members" value="9,640" />
              <PanelRow label="Type" value="Private" />
