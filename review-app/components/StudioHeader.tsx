...
export default function StudioHeader() {
  ...
  // Publish header height as CSS variable so the sidebar can align below it
  useEffect(() => {
  ...
  return (
    <div ref={headerWrapRef} style={{ position: 'sticky', top: 0, zIndex: 40 }}>
      <header className="studio-header">
        ...
        <div className="hidden md:flex items-center gap-1" style={{ position: 'relative' }}>
          ...
          <div style={{ position: 'relative' }}>
            <button className={iconBtn} onClick={() => router.push('/studio/directory')}>
              <Users size={16} />
            </button>
          </div>
        ...
        {protoReady && (
          ...
          >
            ...
            <span style={{
              fontSize: 11, fontWeight: 800, color: '#EAAA00',
        ...
        {connected && publicKey ? (
          <div className="relative shrink-0">
            ...
            {walletMenuOpen && (
              <>
                <div className="fixed inset-0 z-[99]" onClick={() => setWalletMenuOpen(false)} />
                <div
                  className="absolute right-0 mt-2 z-[100] py-1.5 rounded-xl border border-card-border shadow-xl"
                  style={{ minWidth: '200px', background: '#0E0C3A' }}
                >
                  <div className="px-4 py-2.5 border-b border-card-border">
                    <div className="text-[10px] text-muted uppercase tracking-wider mb-1">Connected</div>
                    <div className="text-xs font-mono text-white/80 break-all">{publicKey.toBase58()}</div>
                  </div>
                  <button
                    onClick={handleCopyAddress}
                    className="w-full px-4 py-2.5 text-left text-xs text-white/70 hover:bg-white/[0.04] hover:text-white flex items-center gap-2.5 cursor-pointer transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2.5 text-left text-xs text-red-400 hover:bg-red-500/[0.06] hover:text-red-300 flex items-center gap-2.5 cursor-pointer transition-colors"
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
          ...
          >
           <span className="hidden md:inline">Connect Wallet</span>
            <svg className="md:hidden" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
